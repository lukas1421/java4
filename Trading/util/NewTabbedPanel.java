...
public class NewTabbedPanel extends JPanel {
    ...
    private static final int EXTRA_TAB_WIDTH = 10; 		// extra width on each side
    private static final int BUFF = 20; 	// extra width on each side (should be WID + BUF)
    private static final int TAB_HEIGHT = 15; 	// total height of button 
    ...
    public void addTab(final String titleIn, final JComponent comp, boolean select, boolean canClose) {
        ...
        HtmlButton button = new But(title, canClose, (ActionEvent e) -> {
            select(title);
        });
