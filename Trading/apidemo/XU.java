        ...
        JTable tab = new JTable(m_model) {
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int Index_row, int Index_col) {
                ...
                if (isCellSelected(Index_row, Index_col)) {
                    ...
                    String ticker = ibContractToSymbol(XUTrader.activeFutureCt);
...
//                        graph2.setNavigableMap(indexPriceSina);
        ...
        fillButton.addActionListener(l -> {
            if (priceMapBar.get(ibContractToSymbol(XUTrader.activeFutureCt)).size() > 0 &&
                    priceMapBar.get(FTSE_INDEX).size() > 0) {
                priceMapBar.get(ibContractToSymbol(XUTrader.activeFutureCt)).keySet().forEach(k -> {
