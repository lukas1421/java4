...
public final class AutoTraderXU extends JPanel implements HistoricalHandler, ApiController.IDeepMktDataHandler,
    ...
    //half hour
//    private static volatile EnumMap<HalfHour, AtomicBoolean> manualXUHalfHourDev = new EnumMap<>(HalfHour.class);
//    private static volatile EnumMap<HalfHour, Direction> halfHourXUDevDirection = new EnumMap<>(HalfHour.class);

    private static volatile Map<String, EnumMap<HalfHour, AtomicBoolean>> manualXUHalfHourDev = new ConcurrentHashMap<>();
    //EnumMap<>(HalfHour.class);
    private static volatile Map<String, EnumMap<HalfHour, Direction>> halfHourXUDevDirection = new ConcurrentHashMap<>();
    //new EnumMap<>(HalfHour.class);
    ...
    AutoTraderXU(ApiController ap) {
        ...
        for (FutType f : FutType.values()) {
            ...
            for (HalfHour h : HalfHour.values()) {
                ...
                manualXUHalfHourDev.get(symbol).put(h, new AtomicBoolean(false));
            }
        }
...
//        LocalTime halfHourMinT = getFirstMinTPred(futPrice, t -> t.isAfter(halfHourStart));
        ...
        if (SECONDS.between(lastOrderTime, nowMilli) > waitTimeSec) {
            if (freshPrice > halfHourOpen && !noMoreBuy.get() &&
                    halfHourXUDevDirection.get(symbol).get(h) != Direction.Long) {
                int id = autoTradeID.incrementAndGet();
                ...
                halfHourXUDevDirection.get(symbol).put(h, Direction.Long);
                    ...
                    halfHourXUDevDirection.get(symbol).get(h) != Direction.Short) {
                int id = autoTradeID.incrementAndGet();
    ...
    private static void futPCProfitTaker(LocalDateTime nowMilli, double freshPrice) {
        ...
        double pcSignedQ = getOrderTotalSignedQForTypeFilled(symbol, FUT_OPEN_DEVI);
        ...
        LocalDateTime lastPCTime = getLastOrderTime(symbol, FUT_OPEN_DEVI);
    ...
    private static void sgxA50OpenDeviationTrader(LocalDateTime nowMilli, double freshPrice) {
        ...
        if (!manualfutOpenDevDirection.get()) {
            if (lt.isBefore(ltof(9, 5, 0))) {
                outputDetailedXU(symbol, str(" setting manual fut open dev direction 9:05", lt));
                manualfutOpenDevDirection.set(true);
            } else {
                if (freshPrice > futOpen) {
                    outputDetailedXU(symbol, str(" setting manual fut open dev direction fresh > open", lt));
...
//                    Direction.Long : Direction.Short;
    ...
    public void handle(int errorCode, String errorMsg) {
        outputDetailedXU(ibContractToSymbol(activeFutureCt), "ERROR code " + errorCode + " message " + errorMsg);
