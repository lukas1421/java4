...
//        }
    ...
    static void openDeviationTrader(LocalDateTime nowMilli, double lastIndex) {
        ...
        long tBtwnLastTwoOrders = lastTwoOrderMilliDiff(OPEN_DEVI);
...
//        }
        ...
        if (numOrdersOpenDev % 2 == 0) {
            ...
            msg = " aggressive on closing trades ";
        }
        ...
        if (SECONDS.between(lastOpenDevTradeTime, nowMilli) > waitTimeInSeconds) {
            if (!noMoreBuy.get() && lastIndex > openIndex && openDeviationDirection != Direction.Long) {
                ...
                int id = autoTradeID.incrementAndGet();
                Order o = placeOfferLimit(sellPrice, pmchy < PMCHY_LO ? 1 : sellSize);
                globalIdOrderMap.put(id, new OrderAugmented(nowMilli, o, OPEN_DEVI));
                apcon.placeOrModifyOrder(activeFutureCt, o, new DefaultOrderHandler(id));
    ...
    static void pmHiLoTrader(LocalDateTime nowMilli, double indexLast) {
        ...
        pr(lt.truncatedTo(ChronoUnit.SECONDS)
                , "pm hilo trader: pmOpen, pmFT, T, dir: ", r(pmOpen), r(pmFirstTick), pmFirstTickTime
...
//        }
    ...
    private static synchronized void percentileMATrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        NavigableMap<LocalDateTime, SimpleBar> fut = trimDataFromYtd(futData.get(ibContractToFutType(activeFutureCt)));
        int _2dayPerc = getPercentileForLast(fut);
    ...
    private static int getPDPercentile() {
        ...
        futData.get(ibContractToFutType(activeFutureCt)).entrySet().stream().filter(e -> e.getKey()
