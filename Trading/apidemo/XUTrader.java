    ...
    public static void processMain(LocalDateTime ldt, double price) {
        ...
        if (detailedPrint.get()) {
            pr("20DayMA", _20DayMA, "maxT>MinT: ", maxAfterMin, "max>PrevC", maxAbovePrev,
                    ...
                    , "currDelta ", currDelta, " hilo direction: ", a50IndexDirection);
        }
...
//                                "CNY"), 1.0));
    ...
    private static void futOpenTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (MINUTES.between(lastOpenTime, nowMilli) >= waitTime) {
            if (last > maxP && (_2dayPerc < DOWN_PERC_WIDE || pmchy < 0)) {
                int id = autoTradeID.incrementAndGet();
            ...
            } else if (last < minP && (_2dayPerc > UP_PERC_WIDE || pmchy > 0)) {
                int id = autoTradeID.incrementAndGet();
    ...
    private static void firstTickTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (MINUTES.between(lastOpenTime, nowMilli) >= ORDER_WAIT_TIME) {
            if (ftick2 > open && _2dayPerc < DOWN_PERC_WIDE) {
                int id = autoTradeID.incrementAndGet();
            ...
            } else if (ftick2 < open && _2dayPerc > UP_PERC_WIDE) {
                int id = autoTradeID.incrementAndGet();
    ...
    private static void chinaHiLoTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (_2dayFutPerc > UP_PERC_WIDE) {
            sellSize = 2;
        } else if (_2dayFutPerc < DOWN_PERC_WIDE) {
            buySize = 2;
        }
    ...
    private static void intraday1stTickAccumulator(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (MINUTES.between(lastOpenTime, nowMilli) >= ORDER_WAIT_TIME * 2) {
            if (firstTick > open && lt.isBefore(LocalTime.of(11, 30)) && _2dayFutPerc < 5 && pmchy < 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeBidLimit(freshPrice, 1);
            ...
            } else if (firstTick < open && lt.isBefore(LocalTime.of(11, 30)) && _2dayFutPerc > 95 && pmchy > 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeOfferLimit(freshPrice, 1);
                globalIdOrderMap.put(id, new OrderAugmented(nowMilli, o, INTRADAY_FIRSTTICK_ACCU));
                apcon.placeOrModifyOrder(activeFuture, o, new DefaultOrderHandler(id));
