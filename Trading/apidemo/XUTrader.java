...
//                                "CNY"), 1.0));
    ...
    private static void futOpenTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (last > maxP && pmchy < 0 && _2dayPerc < DOWN_PERC_WIDE) {
        ...
        } else if (last < minP && pmchy > 0 && _2dayPerc > UP_PERC_WIDE) {
    ...
    private static void chinaHiLoTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (SECONDS.between(lastHiLoTradeTime, nowMilli) >= 60) {
            if (lastV > maxSoFar && ftseDirection == Direction.Short && pmchy < 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeBidLimit(freshPrice, 1);
                ...
                ftseDirection = Direction.Long;
            } else if (lastV < minSoFar && ftseDirection == Direction.Long && pmchy > 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeOfferLimit(freshPrice, 1);
                globalIdOrderMap.put(id, new OrderAugmented(nowMilli, o, AutoOrderType.CHINA_HILO));
                apcon.placeOrModifyOrder(activeFuture, o, new DefaultOrderHandler(id));
