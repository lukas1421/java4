...
public final class XUTrader extends JPanel implements HistoricalHandler, ApiController.IDeepMktDataHandler,
    ...
    //china open
    private static volatile Direction chinaOpenDirection = Direction.Flat;
...
//                                "CNY"), 1.0));
    ...
    private static void futOpenTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        double maxP = futPrice.entrySet().stream().mapToDouble(Map.Entry::getValue).max().orElse(0.0);
        double minP = futPrice.entrySet().stream().mapToDouble(Map.Entry::getValue).min().orElse(0.0);

        double secLast = futPrice.lowerEntry(futPrice.lastKey()).getValue();
        ...
        if (last == maxP && maxP > secLast && pmchy < 0 && _2dayPerc < DOWN_PERC_WIDE) {
            if (MINUTES.between(lastOpenTime, nowMilli) >= buyWaitTime) {
                ...
                outputOrderToAutoLog(str(o.orderId(), "fut open buy",
                        globalIdOrderMap.get(id), " max min last ", maxP, minP, last));
            }
        } else if (last == minP && minP < secLast && pmchy > 0 && _2dayPerc > UP_PERC_WIDE) {
            if (MINUTES.between(lastOpenTime, nowMilli) >= sellWaitTime) {
                int id = autoTradeID.incrementAndGet();
    ...
    private static void firstTickTrader(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        if (MINUTES.between(lastOpenTime, nowMilli) >= ORDER_WAIT_TIME) {
            if (curr > open && pmchy < 0 && _2dayPerc < DOWN_PERC_WIDE) {
                int id = autoTradeID.incrementAndGet();
            ...
            } else if (curr < open && pmchy > 0 && _2dayPerc > UP_PERC_WIDE) {
                int id = autoTradeID.incrementAndGet();
    ...
    private static void chinaHILOTrader(LocalDateTime nowMilli, double freshPrice) {
        ...
        if (lastV > maxSoFar && chinaOpenDirection == Direction.Short && _2dayFutPerc < DOWN_PERC_WIDE) {
            int id = autoTradeID.incrementAndGet();
            ...
            chinaOpenDirection = Direction.Long;
        } else if (lastV < minSoFar && chinaOpenDirection == Direction.Long && _2dayFutPerc > UP_PERC_WIDE) {
            int id = autoTradeID.incrementAndGet();
    ...
    private static void closeProfitTaker(LocalDateTime nowMilli, double freshPrice) {
        ...
        LocalDateTime lastCloseProfitTaker = getLastOrderTime(AutoOrderType.CLOSE_TAKE_PROFIT);

        if (MINUTES.between(lastCloseProfitTaker, nowMilli) >= ORDER_WAIT_TIME) {
            if (todayPerc < 10 && firstTick < open) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeBidLimit(freshPrice, CONSERVATIVE_SIZE);
            ...
            } else if (todayPerc > 90 && firstTick > open) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeOfferLimit(freshPrice, CONSERVATIVE_SIZE);
                globalIdOrderMap.put(id, new OrderAugmented(nowMilli, o, CLOSE_TAKE_PROFIT));
                apcon.placeOrModifyOrder(activeFuture, o, new DefaultOrderHandler(id));
    ...
    private static void intraday1stTickAccumulator(LocalDateTime nowMilli, double freshPrice, int pmchy) {
        ...
        pr(" intraday first tick, open, firstTick, futP% ", open, firstTick, _2dayFutPerc);

        if (MINUTES.between(lastOpenTime, nowMilli) >= ORDER_WAIT_TIME) {
            if (firstTick > open && _2dayFutPerc < 5 && lt.isBefore(LocalTime.of(11, 30)) && pmchy < 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeBidLimit(freshPrice, 1);
            ...
            } else if (firstTick < open && _2dayFutPerc > 95 && lt.isBefore(LocalTime.of(11, 30)) && pmchy > 0) {
                int id = autoTradeID.incrementAndGet();
                Order o = placeOfferLimit(freshPrice, 1);
                globalIdOrderMap.put(id, new OrderAugmented(nowMilli, o, INTRADAY_FIRSTTICK_ACCU));
                apcon.placeOrModifyOrder(activeFuture, o, new DefaultOrderHandler(id));
    ...
    private static synchronized void percentileMATrader(LocalDateTime nowMilli, double freshPrice, int pmPercChg) {
        ...
        double pmchgDelta = (pmPercChg < 0 ? 1 : -1) * PMCHY_DELTA * Math.abs(pmPercChg) / 100.0;
